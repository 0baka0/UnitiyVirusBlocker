	...
	VBSceneInstance
{
	[Header("시작 전 딜레이")]
	[Range(0.0f, 10.0f)]
	[SerializeField] private float _StageBeginDelay = 3.0f;
    ...
    public Dictionary<EnemyType, EnemyCharacter> _EnemyPrefabs = new Dictionary<EnemyType, EnemyCharacter>();
    ...
    protected override void Awake()
    {
        ...
        _EnemyPrefabs.Add(EnemyType.Enemy1, ResourceManager.Instance.LoadResource<GameObject>(
            "Enemy1",
            "Prefabs/Characters/EnemuCharacter/Enemy1").GetComponent<EnemyCharacter>());

        _EnemyPrefabs.Add(EnemyType.Enemy2, ResourceManager.Instance.LoadResource<GameObject>(
            "Enemy2",
            "Prefabs/Characters/EnemuCharacter/Enemy2").GetComponent<EnemyCharacter>());

        _EnemyPrefabs.Add(EnemyType.Enemy3, ResourceManager.Instance.LoadResource<GameObject>(
            "Enemy3",
            "Prefabs/Characters/EnemuCharacter/Enemy3").GetComponent<EnemyCharacter>());
    ...
    private void StartStage()
    {
        ...
        // 스테이지 시작 전 딜레이를 줍니다.
        IEnumerator StartBeginDelay()
        {
            WaitForSeconds waitSec = new WaitForSeconds(_StageBeginDelay);
        ...
        StartCoroutine(StartBeginDelay());
    }
    ...
    private void SpawnEnemy(StageWaveInfo stageInfo, List<EnemyCharacter> spawnedEnemies)
    {
        ...
        for (int i = 0; i < stageInfo.enemyType1; ++i)
            spawnedEnemies.Add(SpawnEnemy(EnemyType.Enemy1));
        for (int i = 0; i < stageInfo.enemyType2; ++i)
            spawnedEnemies.Add(SpawnEnemy(EnemyType.Enemy2));
        for (int i = 0; i < stageInfo.enemyType3; ++i)
            spawnedEnemies.Add(SpawnEnemy(EnemyType.Enemy3));
    }
