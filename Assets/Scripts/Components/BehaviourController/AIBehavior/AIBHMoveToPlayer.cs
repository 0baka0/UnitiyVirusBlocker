    ...
    AIBehaviorBase
{
    ...
    {
        _LastDirectionUpdateTime = Time.time - _DirectionUpdateDelay;
    ...
    // 이동 방향을 갱신합니다.
    private void UpateDirection()
    {
        if (Time.time - _LastDirectionUpdateTime >= _DirectionUpdateDelay)
        {
    ...
    public override void Run()
    {
        ...
        IEnumerator Behavior()
        {
            while (true)
            {
                // 플레이어 캐릭터와 거리가 가깝다면 행동 종료
                if (Vector3.Distance(transform.position, _PlayerableCharacter.transform.position) < 0.1f)
                    break;
